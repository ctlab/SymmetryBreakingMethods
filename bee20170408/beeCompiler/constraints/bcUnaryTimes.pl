...
upostimesSimplify(Constr, NewConstr, Changed):-!,
     ...
     (Changed==1 ->
         ...
         (specialCasesPos(NNA,NNB,NNC,NewConstr) ;
         (DomChanged==1 ->
...
% |    Special Cases      |
% -------------------------
specialCasesPos(A,B,C,Constr):-!,
    (A=(Aval,Aval,_) ->
        Constr=none,
        auxUnarynum:unarynumMulByConst(B,Aval,FB),
        auxUnarynum:unarynumEquals(FB,C) ;
    (B=(Bval,Bval,_) ->
        Constr=none,
        auxUnarynum:unarynumMulByConst(A,Bval,FA),
        auxUnarynum:unarynumEquals(FA,C) ;
    (C=(Cval,Cval,_) ->
        (Cval > 0 ->
            Constr=none,
            unaryPosTimesEqK(A,B,Cval)
        ;
            unaryPosTimesEq0(A,B,Constr)
        ) ;
    !,fail
    ))).
